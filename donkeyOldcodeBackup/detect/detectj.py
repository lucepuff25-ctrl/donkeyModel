...
def execute_warning_action():
    """执行警告动作（阻塞式）- 不停止车道线检测"""
    print("============== 执行警告动作 ==============")
    # 移除 flag = 0 的设置，保持车道线检测运行
    send_serial_data(0x80, 0xE0)
    time.sleep(2.0)  # 阻塞2秒
    print("[ACTION] 警告动作完成")
...
def execute_limitation_action():
    ...
    time.sleep(2)
    # 执行2秒右转
    send_serial_data(0x70, 0x05)  # 限速 + 右转
    time.sleep(2.0)  # 阻塞2秒
...
def process_lane(frame):
    ...
    angle_deg = math.degrees(angle_rad) * 0.8
        ...
        if lane_mode:
            ...
            if results.boxes and results.boxes.cls.numel():
                ...
                for i, (box, cls_id, conf) in enumerate(zip(boxes, clses, confs)):
                    ...
                    if name == "limitation" and area_ratio >= 0.018:
                ...
                if not any([limitation_detected, warning_detected, passing_bay_detected,
                            ...
                            turn_left_detected, pedestrain_passagemay_detected, lift_detected]):
                    current_angle = get_global_angle()
                    current_speed = limited_speed if speed_limit_active else normal_speed
                    send_serial_data(current_speed, current_angle)
                else:
                    # 检测到交通标志时，不发送角度数据，但车道线仍在计算
                    print("[INFO] 检测到交通标志，暂停发送角度数据")
