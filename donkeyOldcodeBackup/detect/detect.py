...
ser_data = bytearray([0xAA, 0x80, 0x80, 0xFF])
shutdown_flag = False
...
def start_turn_action(turn_type_name):
    """开始转向动作（非阻塞）"""
    global turn_in_progress, turn_frame_count, turn_total_frames, turn_type, roundabout_mode, roundabout_counter,wait_for_first_turn

    print(f"[TURN] 开始执行{turn_type_name}动作...")



    if turn_type_name == "trun_left":
        send_serial_data(0x80, 0xFF)  # 左转
        time.sleep(1)
    ...
    elif turn_type_name == "trun_right":
        send_serial_data(0x80, 0xFF)  # 右转
        time.sleep(1)
        wait_for_first_turn=False
        turn_in_progress=False
        print(f"=====success=====")
...
def execute_warning_action():
    """执行警告动作（阻塞式）- 不停止车道线检测"""
    print("============== 执行警告动作 ==============")
    send_serial_data(0x80, 0xFF)
    time.sleep(0.8)  # 阻塞2秒
    print("[ACTION] 警告动作完成")


def execute_passing_bay_action():
    """执行避让动作（阻塞式）- 不停止车道线检测"""
    print("============== 执行避让动作 ==============")
    send_serial_data(0x80, 0x00)
    time.sleep(1.0)  # 阻塞2秒
    print("[ACTION] 避让动作完成")


def execute_zebra_crossing_action():
    """执行人行横道动作（阻塞式）- 不停止车道线检测"""
    global flag_zebra_line, roundabout_mode, roundabout_counter, left_line_lost_count
...
def execute_limitation_action():
    """执行限速标志动作：2秒右转并使用限速（阻塞式）- 不停止车道线检测"""
    print("============== 执行限速标志动作 ==============")
    time.sleep(0.8)
    # 执行2秒右转
    send_serial_data(0x70, 0x00)  # 限速 + 右转
    time.sleep(1)  # 阻塞2秒

    print("[ACTION] 限速标志动作完成，进入限速模式")


def start_roundabout_turn_action():
    """开始环岛转向动作（阻塞式）- 不停止车道线检测"""
    print("[ROUNDABOUT] 开始环岛转向")
    # 移除 flag = 0 的设置
    send_serial_data(0x80, 0xFF)
    time.sleep(0.6)  # 阻塞0.5秒
    print("[ROUNDABOUT] 环岛转向完成")
...
def process_lane(frame):
    global current_state, last_known_track_width, TARGET_OFFSET, track_width_ema, two_lane_streak,angle_byte

    # 1. 启动阶段：固定直行
    global wait_for_first_turn, turn_in_progress, red_light_stop,red_one
    if wait_for_first_turn:
        print("[LANE] 启动阶段：固定直行中，等待转向标志...")
        return frame

    # 2. 转向进行中时，跳过车道线检测
    if turn_in_progress:
        print("[LANE] 转向进行中，跳过车道线检测")
        return frame

    if red_light_stop and red_one:
    	print("[LANE] red light stop")
    	send_serial_data(0x00, 0x80)
    	return frame    
    #3. 暂停标志
    with flag_lock:
    ...
    if left_fit is not None and right_fit is not None:
        ...
        valley = np.min(hist[min(leftx_base, rightx_base): max(leftx_base, rightx_base)]) if rightx_base>leftx_base else np.min(hist[max(rightx_base,leftx_base):min(rightx_base,leftx_base)])
    ...
    angle_byte = int((45 - angle_deg) * 255 / 90) & 0xFF
    ...
    # 绘制ROI区域边界（红色线条）
    cv2.polylines(color_warp, [pts], True, (0, 0, 255), 3)  # 绘制ROI梯形边界（红色，3像素粗）
    
    # 在ROI顶点添加标记点
    for i, point in enumerate(pts[0]):
        cv2.circle(color_warp, tuple(point), 5, (0, 0, 255), -1)  # 红色圆点标记顶点
        cv2.putText(color_warp, f"P{i+1}", (point[0]+5, point[1]-5), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)  # 标记顶点编号

    # 调试信息显示
    
    # 在图像上显示角度信息
    direction = "左转" if angle_deg < -5 else "右转" if angle_deg > 5 else "直行"
    byte_direction = "左拐" if angle_byte > 128 else "右拐" if angle_byte < 128 else "直行"
    cv2.putText(color_warp, f"Angle:{angle_deg:.1f}*", (10, h - 100),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    cv2.putText(color_warp, f"Dir:{direction}", (10, h - 80),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0) if angle_deg < -5 else (0, 255, 0) if angle_deg > 5 else (255, 255, 0), 2)
    cv2.putText(color_warp, f"Byte:{angle_byte:3d}(0x{angle_byte:02X})", (10, h - 60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(color_warp, f"Cmd:{byte_direction}", (10, h - 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255) if angle_byte > 128 else (0, 255, 255) if angle_byte < 128 else (255, 255, 0), 2)
    ...
    cv2.putText(color_warp, f"Single:{'ON' if FORCE_SINGLE_SIDE else 'OFF'}", (400, 60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
    cv2.putText(color_warp, f"Curvature:{current_curvature:.6f}", (400, 80),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    cv2.putText(color_warp, f"Colors:Y:{yellow_pixels} W:{white_pixels} R:{red_pixels}", (400, 100),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 2)
    cv2.putText(color_warp, f"angle_byte:{angle_byte}", (400, 120),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (1, 255, 255), 2)
    
    # 在虚拟中心线上添加标记点 - 只在ROI区域内绘制
    target_y = int(h * set_hight)
    if roi_start_y <= target_y <= roi_end_y:
        target_x = int(center_x[target_y])
        cv2.circle(color_warp, (target_x, target_y), 10, (0, 255, 255), -1)  # 黄色圆点标记虚拟中心线目标点
        cv2.putText(color_warp, "VIRTUAL", (target_x + 15, target_y - 5),
...
def infer_thread(model, q_in, q_out, lane_mode):
    global wait_for_first_turn, turn_in_progress, flag_zebra_line, red_light_stop
        ...
        if lane_mode:
            ...
            if results.boxes and results.boxes.cls.numel():
                ...
                for i, (box, cls_id, conf) in enumerate(zip(boxes, clses, confs)):
                    ...
                    if name == "limitation" and area_ratio >=0.003:
                        ...
                        print(f"[AREA] {name}: {area_ratio:.4f}")
                    #elif name == "trun_right" and area_ratio >= 0.002:
                        #turn_left_detected = True
                        #print(f"[AREA] {name}: {area_ratio:.4f}")
                    elif name == "trun_left" and area_ratio >= 0.012:
                        turn_left_detected = True
                        print(f"[AREA] {name}: {area_ratio:.4f}")
                    elif name == "PPC" and area_ratio >= 0.012:
                        pedestrain_passagemay_detected = True
                        print(f"[AREA] {name}: {area_ratio:.4f}")
                    elif name == "lift" and area_ratio >= 0.01:  # 解除限速标志
                        lift_detected = True
                        print(f"[AREA] {name}: {area_ratio:.4f}")
                ...
                if not wait_for_first_turn and not turn_in_progress:
                    # 阻塞式动作处理
                    if warning_detected and can_execute_task("warning"):
                        print("============== 检测到警告标志 ==============")
                        mark_task_executed("warning")
                        execute_warning_action()

                    elif passing_bay_detected and can_execute_task("passing_bay"):
                        print("============== 检测到避让标志 ==============")
                        mark_task_executed("passing_bay")
                        execute_passing_bay_action()

                    elif pedestrain_passagemay_detected and can_execute_task("PPC"):
                        print("============== 检测到人行横道 ==============")
                        mark_task_executed("PPC")
                        execute_zebra_crossing_action()

                    elif limitation_detected and can_execute_task("limitation") and not speed_limit_active:
                        print("============== 检测到限速标志 ==============")
                        mark_task_executed("limitation")
                        execute_limitation_action()
                        speed_limit_active = True

                    elif lift_detected and can_execute_task("lift") and speed_limit_active:
                        print("============== 检测到解除限速标志 ==============")
                        mark_task_executed("lift")
                        speed_limit_active = False
                        if not red_light_stop:
                            send_serial_data(normal_speed, 0x80)
                        print("[ACTION] 解除限速，恢复正常速度")
